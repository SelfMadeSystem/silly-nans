...
function createBlob({
  ...
  return {
    width,
    height,
    speedX: randomInt(minSpeedX, maxSpeedX),
    speedY: randomInt(minSpeedY, maxSpeedY),
...
function createBlobs(count: number, options: Options): Blob[] {
  return Array.from({ length: count }, createBlob.bind(null, options));
}
...
function generateCssBlobs(options?: Options): string {
...
export default function BlobBg({ options }: { options?: Options }) {
